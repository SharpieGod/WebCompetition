            ...
            if child_user is not None:
                if check_password_hash(child_user.password, child_password):
                    if check_password_hash(current_user.password, parent_password):
                        if (
                            ParentRelationship.query.filter_by(
                                child_id=child_user.id
                            ).first()
                            is None
                        ):
                            flash(
                                f"Added {child_first_name} to children list successfully!",
                                category="success",
                            )
                            ...
                            flash(
                                "Parent relationship already exists", category="error"
                            )
                    else:
                ...
                flash(
                    "Child email address or first name or last name are incorrect.",
                    category="error",
                )
