...
def manage_child():
    if current_user.parent:
        if (
            ParentRelationship.query.filter_by(parent_id=current_user.id).first()
            is None
        ):
...
def grades():
    if not current_user.parent:
        ...
        for grade_relationship in grade_relationships:
            grades.append(Grade.query.filter_by(id=grade_relationship.grade_id).first())
...
def add_grade():
    ...
    grades = [GradeEnum.E, GradeEnum.A, GradeEnum.D, GradeEnum.B, GradeEnum.I]
    subject_options = [
        "Math",
        "Science",
        "English",
...
def manage(child_id):
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("views.manage", **args.to_dict(flat=False), child_id=child_id)
        )

    if current_user.parent:
        children = [
            x for x in ParentRelationship.query.filter_by(parent_id=current_user.id)
        ]
        ...
        if ParentRelationship.query.filter_by(child_id=child.id).first():
            if (
                ParentRelationship.query.filter_by(child_id=child_id).first().parent_id
                != current_user.id
            ):
                return redirect(url_for("views.manage_child"))
        else:
        ...
        grades = [
            Grade.query.filter_by(id=x.grade_id).first() for x in grade_relationships
        ]
        ...
        if grades != []:
            ...
            grade_avg = sum(
                [grade_value.get(x) for x in [y.grade.value for y in grades]]
            ) / len(grades)
...
def edit_grade(grade_id):
    ...
    grade_relationship = GradeRelationship.query.filter_by(grade_id=grade_id).first()
    ...
    child_id = (
        User.query.filter_by(
            id=GradeRelationship.query.filter_by(grade_id=grade.id).first().child_id
        )
        .first()
        .id
    )
    ...
    if request.method == "POST":
        ...
        if request_grade not in grade_options or subject not in subject_options:
            ...
            flash(
                "Grade comment cannot be longer than 120 characters", category="error"
            )
        else:
...
def parent_add_grade(child_id: int):
    ...
    if ParentRelationship.query.filter_by(child_id=child.id).first():
        if (
            ParentRelationship.query.filter_by(child_id=child.id).first().parent_id
            != current_user.id
        ):
            return redirect(url_for("views.manage_child"))
    else:
...
def delete_grade():
    ...
    grade_relationship = GradeRelationship.query.filter_by(grade_id=grade.id).first()
    parent_relationship = ParentRelationship.query.filter_by(
        child_id=grade_relationship.child_id
    ).first()
